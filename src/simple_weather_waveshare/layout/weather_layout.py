...
class WeatherLayout(DisplayInterface):
    def get_display_output(self) -> Image:
        ...
        weather = Image.new(mode='RGB', size=(int(self.screen_length), int(self.screen_height / 2)), color=(255, 255, 255))
        d2 = ImageDraw.Draw(weather)

        legend = 'C°\nmm'
        d2.multiline_text(xy=(int(self.screen_length / 8), int(self.screen_height / 4)), text=legend, font=self.get_font(35), fill=(0, 0, 0), anchor="mm")

        for i in range(len(weather_data[0])):
            weather_text = f'{weather_data[0][i]}\n{weather_data[1][0]}'
            d2.multiline_text(xy=(int(self.screen_length / 8 * (i + 2)), int(self.screen_height / 4)), text=weather_text, font=self.get_font(35), fill=(0, 0, 0), anchor="mm")
